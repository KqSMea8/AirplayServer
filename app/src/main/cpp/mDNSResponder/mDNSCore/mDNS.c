...
mDNSlocal void GenerateNegativeResponse(mDNS *const m, mDNSInterfaceID InterfaceID, QC_result qc)
{
    ...
    q = m->CurrentQuestion;
    LogInfo("[R%d->Q%d] GenerateNegativeResponse: Generating negative response for question %##s (%s)", 
            q->request_id, mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
...
mDNSexport void AnswerQuestionByFollowingCNAME(mDNS *const m, DNSQuestion *q, ResourceRecord *rr)
{
    ...
    {
        LogMsg("[R%d->Q%d] AnswerQuestionByFollowingCNAME: %p %##s (%s) NOT following CNAME referral %d%s for %s",
               q->request_id, mDNSVal16(q->TargetQID), q, q->qname.c, DNSTypeName(q->qtype), q->CNAMEReferrals, selfref ? " (Self-Referential)" : "", RRDisplayString(m, rr));
    }
    else
    {
        ...
        LogInfo("[R%d->Q%d] AnswerQuestionByFollowingCNAME: %p %##s (%s) following CNAME referral %d for %s",
                q->request_id, mDNSVal16(q->TargetQID), q, q->qname.c, DNSTypeName(q->qtype), q->CNAMEReferrals, RRDisplayString(m, rr));
        ...
        {
            LogInfo("[R%d->Q%d] AnswerQuestionByFollowingCNAME: Resolving a .local CNAME %p %##s (%s) Record %s",
                    q->request_id, mDNSVal16(q->TargetQID), q, q->qname.c, DNSTypeName(q->qtype), RRDisplayString(m, rr));
...
mDNSlocal void AnswerNewQuestion(mDNS *const m)
{
    ...
    m->lock_rrcache = 1;
    if (m->CurrentQuestion)
        LogMsg("[R%d->Q%d] AnswerNewQuestion ERROR m->CurrentQuestion already set: %##s (%s)",
    ...
    {
        LogMsg("[R%d->Q%d] AnswerNewQuestion: NoAnswer_Fail %##s (%s)", q->request_id, mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
    ...
    {
        LogInfo("[R%d->Q%d] AnswerNewQuestion: Generating response for retrying with search domains %##s (%s)", 
                q->request_id, mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
    ...
    if (q->allowExpired == AllowExpired_AllowExpiredAnswers)
    {
        ...
        {
            LogMsg("[R%d->Q%d] AnswerNewQuestion: Restarting original question %p firstExpiredQname %##s for allowExpiredAnswers question", q->request_id, mDNSVal16(q->TargetQID), q, &q->firstExpiredQname.c);
            mDNS_StopQuery_internal(m, q);                              // Stop old query
                                       ...
                                       const mDNSInterfaceID InterfaceID)
{
    ...
    else
    {
            ...
            if (ptr && (qptr = ExpectingUnicastResponseForQuestion(m, dstport, response->h.id, &q, !dstaddr)))
            {
                if (!failure)
                {
                    ...
                    if (!mDNSOpaque16IsZero(response->h.id))
                    {
                        ...
                        {
                            LogInfo("[R%d->Q%d] mDNSCoreReceiveResponse: Setting aware for %##s (%s) on %#a", 
                                    qptr->request_id, mDNSVal16(qptr->TargetQID), 
                                    qptr->qname.c, DNSTypeName(qptr->qtype), &qptr->qDNSServer->addr);
                    ...
                    for (rr = cg ? cg->members : mDNSNULL; rr; rr=rr->next)
                        if (SameNameRecordAnswersQuestion(&rr->resrec, qptr))
                        {
                            ...
                            {
                                LogInfo("[R%d->Q%d] mDNSCoreReceiveResponse: CRDNSSECQuestion set for record %s, question %##s (%s)",
                ...
                else
                {
                    if (qptr)
                    {
                        ...
                        {
                            LogInfo("[R%d->Q%d] mDNSCoreReceiveResponse: Server %p responded with code %d to DNSSEC Query %##s (%s), clear DO flag",
                                    qptr->request_id, mDNSVal16(qptr->TargetQID), qptr->qDNSServer, rcode, q.qname.c, DNSTypeName(q.qtype));
                            qptr->qDNSServer->req_DO = mDNSfalse;
                        }
                        // For Unicast DNS Queries, penalize the DNSServer
                        else
                        {
                            LogInfo("[R%d->Q%d] mDNSCoreReceiveResponse: Server %p responded with code %d to query %##s (%s)",
                                    qptr->request_id, mDNSVal16(qptr->TargetQID), qptr->qDNSServer, rcode, q.qname.c, DNSTypeName(q.qtype));
        ...
        if (returnEarly)
        {
            LogInfo("[Q%d] Ignoring %2d Answer%s %2d Authorit%s %2d Additional%s", mDNSVal16(response->h.id),
                    response->h.numAnswers,     response->h.numAnswers     == 1 ? ", " : "s,",
                    response->h.numAuthorities, response->h.numAuthorities == 1 ? "y,  " : "ies,",
    ...
    for (i = 0; i < totalrecords && ptr && ptr < end; i++)
    {
            ...
            else if (!AcceptableResponse || !dstaddr)
            {
                ...
                if (q != mDNSNULL)
                {
                    ...
                    else
                    {
                        ...
                        LogInfo("[R%d->Q%d] mDNSCoreReceiveResponse: Accepting response for query: %##s (%s)", q->request_id, mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
                    }
                }
                else
                {
        ...
        if (!AcceptableResponse) LogInfo("[Q%d] mDNSCoreReceiveResponse ignoring %s", mDNSVal16(response->h.id), CRDisplayString(m, &m->rec.r));
        if (m->rrcache_size && AcceptableResponse)
        {
            ...
            if (!rr && m->rec.r.resrec.rroriginalttl > 0)
            {
                ...
                if (rr)
                {
                    ...
                    if (unicastQuestion && DNSSECQuestion(unicastQuestion))
                    {
                        LogInfo("[R%d->Q%d] mDNSCoreReceiveResponse: CRDNSSECQuestion set for new record %s, question %##s (%s)",
                                unicastQuestion->request_id, mDNSVal16(unicastQuestion->TargetQID), CRDisplayString(m, rr),
                                unicastQuestion->qname.c, DNSTypeName(unicastQuestion->qtype));
                        rr->CRDNSSECQuestion = 1;
                    }
    ...
    while (CacheFlushRecords != (CacheRecord*)1)
    {
        ...
        if (r1->DelayDelivery)  // If we were planning to delay delivery of this record, see if we still need to
        {
            ...
            if (response->h.numAnswers && unicastQuestion && NSECRecords)
            {
                if (!NSECCachePtr)
                {
                    LogInfo("[R%d->Q%d] mDNSCoreReceiveResponse: Updating NSECCachePtr to %s", 
...
mDNSlocal void UpdateQuestionDuplicates(mDNS *const m, DNSQuestion *const question)
{
    ...
    if (question->DuplicateOf)
    {
        LogInfo("[R%d->DupQ%d->Q%d] UpdateQuestionDuplicates: question %p %##s (%s) duplicate of %p %##s (%s)",
...
mDNSexport DNSServer *GetServerForQuestion(mDNS *m, DNSQuestion *question)
{
    ...
    if (curmatch != mDNSNULL)
    {
        LogInfo("[R%d->Q%d] GetServerForQuestion: %p DNS server (%p) %#a:%d (Penalty Time Left %d) (Scope %s:%p:%d) for %##s (%s)",
                question->request_id, mDNSVal16(question->TargetQID), question, curmatch, &curmatch->addr, mDNSVal16(curmatch->port),
    ...
    {
        LogInfo("[R%d->Q%d] GetServerForQuestion: %p no DNS server (Scope %s:%p:%d) for %##s (%s)",
            question->request_id, mDNSVal16(question->TargetQID), question, ifname ? ifname : "None", InterfaceID, question->ServiceID, name, DNSTypeName(question->qtype));
    }
...
mDNSlocal mDNSBool ShouldSuppressUnicastQuery(mDNS *const m, DNSQuestion *q, DNSServer *d)
{
    ...
    {
        LogDebug("[Q%d] ShouldSuppressUnicastQuery: Query not suppressed for %##s, qtype %s, not A/AAAA type", mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
        return mDNSfalse;
    }
    ...
    {
        LogDebug("[Q%d] ShouldSuppressUnicastQuery: Query not suppressed for %##s, qtype %s, Private Domain", mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
        return mDNSfalse;
    }

    if (!d)
    {
        LogInfo("[Q%d] ShouldSuppressUnicastQuery: Query suppressed for %##s, qtype %s, as the DNS server is NULL", mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
        return mDNStrue;
    }
    ...
    {
        LogDebug("[Q%d] ShouldSuppressUnicastQuery: Query not suppressed for %##s, qtype %s, DNSServer %##s %#a:%d allows A queries", mDNSVal16(q->TargetQID), q->qname.c,
                DNSTypeName(q->qtype), d->domain.c, &d->addr, mDNSVal16(d->port));
        return mDNSfalse;
    }
    if ((q->qtype == kDNSType_AAAA) && (d->req_AAAA))
    {
        LogDebug("[Q%d] ShouldSuppressUnicastQuery: Query not suppressed for %##s, qtype %s, DNSServer %##s %#a:%d allows AAAA queries", mDNSVal16(q->TargetQID), q->qname.c,
                DNSTypeName(q->qtype), d->domain.c, &d->addr, mDNSVal16(d->port));
        return mDNSfalse;
    }
#if USE_DNS64
    ...
    {
        LogDebug("[Q%d] ShouldSuppressUnicastQuery: DNS64 query not suppressed for %##s, qtype %s", mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype));
        return mDNSfalse;
    }
#endif

    LogInfo("[Q%d] ShouldSuppressUnicastQuery: Query suppressed for %##s, qtype %s, since DNS Configuration does not allow (req_A is %s and req_AAAA is %s)",
            mDNSVal16(q->TargetQID), q->qname.c, DNSTypeName(q->qtype), d->req_A ? "true" : "false", d->req_AAAA ? "true" : "false");
...
mDNSlocal void FinalizeUnicastQuestion(mDNS *const m, DNSQuestion *question)
{
    ...
    if (question->DuplicateOf)
    {
        ...
        {
            if (question->qDNSServer)
                LogInfo("[R%d->Q%d] FinalizeUnicastQuestion: Current(dup) question %p has DNSServer(%#a:%d) but original question(%p) has no DNS Server! %##s (%s)",
        ...
        LogInfo("[R%d->DupQ%d->Q%d] FinalizeUnicastQuestion: Duplicate question %p (%p) %##s (%s), DNS Server %#a:%d",
                 question->request_id, mDNSVal16(question->DuplicateOf->TargetQID), mDNSVal16(question->TargetQID), question,
                 question->DuplicateOf, question->qname.c, DNSTypeName(question->qtype),
                 question->qDNSServer ? &question->qDNSServer->addr : mDNSNULL,
                 mDNSVal16(question->qDNSServer ? question->qDNSServer->port : zeroIPPort));
    }
...
mDNSexport void mDNSCoreReceiveRawPacket(mDNS *const m, const mDNSu8 *const p, const mDNSu8 *const end, const mDNSInterfaceID InterfaceID)
{
    ...
    else if (end >= p+34 && mDNSSameOpaque16(eth->ethertype, Ethertype_IPv4) && (pkt->v4.flagsfrags.b[0] & 0x1F) == 0 && pkt->v4.flagsfrags.b[1] == 0)
    {
        ...
        debugf("Got IPv4 %02X from %.4a to %.4a", pkt->v4.protocol, (char *)&pkt->v4.src, (char *)&pkt->v4.dst);
    ...
    else if (end >= p+54 && mDNSSameOpaque16(eth->ethertype, Ethertype_IPv6))
    {
        ...
        debugf("Got IPv6  %02X from %.16a to %.16a", pkt->v6.pro, (char *)&pkt->v6.src, (char *)&pkt->v6.dst);
