...
#ifdef DNSSD_RELAY_CLIENT
	if (info && info->ifinfo.discoveryRelayLink != NULL) {
		return rciSendMessage(msg, end, info->ifinfo.discoveryRelayLink);
	}
#endif
...
mDNSlocal void tcpKQSocketCallback(__unused int fd, short filter, void *context, __unused mDNSBool encounteredEOF)
{
    ...
    if (filter == EVFILT_WRITE) 
	{
        ...
        // from the TLS connect code.   However, we will never get here between the first connect callback and
        // the end of TLS startup negotiation because we won't be subscribed to write events during that time.
        
		// If we asked for a writability test, either we are connecting (sock->connected == mDNSFalse) or we are
		// checking to see if we are below the "not sent low water mark."
        if (sock->connected)
        {
            sock->writable = mDNStrue;
        }
...
#ifdef DNSSD_RELAY_CLIENT
// Expose the KQueue event API to code that sits on the platform layer and not the MacOS layer
// The caller is responsible for managing the kQueueEntry data store.  We assume that the
// caller will have allocated two entries, one for read and one for write, which it treats as
// a single object.
extern unsigned mDNSPlatformEventContextSize(void)
{
  return 2 * (sizeof (KQueueEntry));
}

extern mStatus mDNSPlatformRequestReadEvents(int fd, const char *taskName, mDNSPlatformEventCallback callback,
											 void *context, void *eventContext)
{
	KQueueEntry *kqe = eventContext;
	kqe->KQcallback = callback;
	kqe->KQcontext = context;
	kqe->KQtask = taskName;
	mStatus ret = KQueueSet(fd, EV_ADD, EVFILT_READ, kqe);
	if (ret != 0)
		return mStatus_UnknownErr;
	return mStatus_NoError; // I don't think anybody cares about status beyond succeed/fail, but...
}

extern mStatus mDNSPlatformStopReadEvents(int fd, void *eventContext)
{
	mStatus ret = KQueueSet(fd, EV_DELETE, EVFILT_READ, (KQueueEntry *)eventContext);
	if (ret != 0)
		return mStatus_UnknownErr;
	return mStatus_NoError;
}

extern mStatus mDNSPlatformRequestWriteEvents(int fd, const char *taskName, mDNSPlatformEventCallback callback,
											  void *context, void *eventContext)
{
	KQueueEntry *kqe = ((KQueueEntry *)eventContext) + 1;
	kqe->KQcallback = callback;
	kqe->KQcontext = context;
	kqe->KQtask = taskName;
	int ret = KQueueSet(fd, EV_ADD | EV_ONESHOT, EVFILT_WRITE, kqe);
	if (ret != 0)
		return mStatus_UnknownErr;
	return mStatus_NoError;
}

extern mStatus mDNSPlatformStopWriteEvents(int fd, void *eventContext)
{
	int ret = KQueueSet(fd, EV_ADD, EVFILT_WRITE, ((KQueueEntry *)eventContext) + 1);
	if (ret != 0)
		return mStatus_UnknownErr;
	return mStatus_NoError;
}

extern mStatus mDNSPlatformRemoveFDFromEventLoop(int fd, void *eventContext)
{
	mStatus first, second;
	first = mDNSPlatformStopWriteEvents(fd, eventContext);
	second = mDNSPlatformStopReadEvents(fd, eventContext);
	if (first != mStatus_NoError || second != mStatus_NoError)
		return mStatus_UnknownErr;
	return first;
}
#endif
...
mDNSlocal mStatus SetupTCPSocket(TCPSocket *sock, mDNSAddr_Type addrtype, mDNSIPPort *port, mDNSBool useBackgroundTrafficClass)
{
    ...
    int skt = socket(addrtype == mDNSAddrType_IPv4 ? AF_INET : AF_INET6, SOCK_STREAM, IPPROTO_TCP);
    ...
    if (addrtype == mDNSAddrType_IPv4)
    {
		...
		// XXX How would this work, given that we never call recvmsg() on a TCP socket?
...
mDNSexport TCPSocket *mDNSPlatformTCPSocket(TCPSocketFlags flags, mDNSAddr_Type addrtype, mDNSIPPort *port, domainname *hostname, mDNSBool useBackgroundTrafficClass)
{
    ...
    TCPSocket *sock = mallocL("TCPSocket/mDNSPlatformTCPSocket", len);
...
mDNSexport mStatus mDNSPlatformTCPConnect(TCPSocket *sock, const mDNSAddr *dst, mDNSOpaque16 dstport, mDNSInterfaceID InterfaceID, TCPConnectionCallback callback, void *context)
{
	...
	// If the socket connected synchronously, we may not get an EVFILT_WRITE event, so we need to set up
	// the read event here and not count on the EVFILT_WRITE event to trigger the flow that sets it up.
	if (sock->err == mStatus_NoError &&
		KQueueSet(sock->fd, EV_ADD, EVFILT_READ, &sock->kqEntry))
	{
		// And of course if that fails, we can't use the connection even though we have it.
...
mDNSexport long mDNSPlatformWriteTCP(TCPSocket *sock, const char *msg, unsigned long len)
{
	...
	// See if we are still below the low water mark.
    sock->writable = mDNSfalse;
    if (KQueueSet(sock->fd, EV_ADD /* | EV_ONESHOT */, EVFILT_WRITE, &sock->kqEntry))
        LogMsg("ERROR: mDNSPlatformWriteTCP - KQueueSet failed");
...
mDNSlocal NetworkInterfaceInfoOSX *AddInterfaceOrLinkToList(char *name, u_int32_t flags, int family,
															mDNSs32 utc, mDNSu32 scope_id, mDNSEthAddr bssid,
															u_int64_t eflags, mDNSAddr ip, mDNSAddr mask,
															void *link)
{
    mDNS *const m = &mDNSStorage;
    ...
    for (p = &m->p->InterfaceList; *p; p = &(*p)->next)
        if (scope_id == (*p)->scope_id &&
            ...
            (link == NULL && mDNSSameEthAddress(&bssid, &(*p)->BSSID)))
        {
            debugf("AddInterfaceToList: Found existing interface %lu %.6a with address %#a at %p, ifname before %s, after %s", scope_id, &bssid, &ip, *p, (*p)->ifinfo.ifname, name);
            ...
            strlcpy((*p)->ifinfo.ifname, name, sizeof((*p)->ifinfo.ifname));
	...
	if (!link)
		debugf("AddInterfaceOrLinkToList: Making   new   interface %lu %.6a with address %#a at %p",
			   scope_id, &bssid, &ip, i);
	else
		debugf("AddInterfaceOrLinkToList: Making   new   relay link %lu with prefix %#a/%#a at %p",
			   scope_id, &ip, &mask, i);
    ...
    i->ifinfo.ip          = ip;
    i->ifinfo.mask        = mask;
    strlcpy(i->ifinfo.ifname, name, sizeof(i->ifinfo.ifname));
    ...
    i->ifinfo.Advertise   = m->DivertMulticastAdvertisements ? ((flags & IFF_LOOPBACK) ? mDNStrue : mDNSfalse) : m->AdvertiseLocalAddresses;
    i->ifinfo.Loopback    = ((flags & IFF_LOOPBACK) != 0) ? mDNStrue : mDNSfalse;
    i->ifinfo.IgnoreIPv4LL = ((eflags & IFEF_ARPLL) != 0) ? mDNSfalse : mDNStrue;
#ifdef DNSSD_RELAY_CLIENT
	i->ifinfo.discoveryRelayLink = link;
#endif
    ...
    if ((eflags & (IFEF_DIRECTLINK | IFEF_AWDL)) || (strncmp(i->ifinfo.ifname, "p2p", 3) == 0))
        ...
        i->ifinfo.DirectLink  = IsCarPlaySSID(name);

    if (i->ifinfo.DirectLink)
        LogInfo("AddInterfaceToList: DirectLink set for %s", name);
    ...
    if (i->D2DInterface)
        LogInfo("AddInterfaceToList: D2DInterface set for %s", name);
...
#ifdef DNSSD_RELAY_CLIENT
mDNSlocal int AddRelayLinkToList(__unused mDNS *const m, mDNSAddr *addr, mDNSAddr *mask,
							__unused struct sockaddr *intfAddr, char *intfName, int intfIndex, mDNSs32 utc, void *link)
{
	static mDNSEthAddr zero;
	(void)AddInterfaceOrLinkToList(intfName, IFF_MULTICAST, (addr->type == mDNSAddrType_IPv4) ? AF_INET : AF_INET6,
									utc, intfIndex, zero, 0, *addr, *mask, link);
    return 0;
}
#endif

// Code that adds physical interfaces calls this.
mDNSlocal NetworkInterfaceInfoOSX *AddInterfaceToList(struct ifaddrs *ifa, mDNSs32 utc)
{
	...
	return AddInterfaceOrLinkToList(ifa->ifa_name, ifa->ifa_flags, ifa->ifa_addr->sa_family,
									utc, scope_id, bssid, eflags, ip, mask, NULL);
}
    ...
    else
    {
        if (info->AutoTunnelTarget.resrec.RecordType == kDNSRecordTypeUnregistered)
        {
            ...
            mDNS_SetupResourceRecord(&info->AutoTunnelTarget, mDNSNULL, mDNSInterface_Any, kDNSType_A, kHostNameTTL,
                                     kDNSRecordTypeUnregistered, AuthRecordAny, AutoTunnelRecordCallback, info);
            AssignConstStringDomainName(&info->AutoTunnelTarget.namestorage, "\x0B" "_autotunnel");
        ...
        if (info->AutoTunnelService.resrec.RecordType == kDNSRecordTypeUnregistered)
        {
            ...
            mDNS_SetupResourceRecord(&info->AutoTunnelService, mDNSNULL, mDNSInterface_Any, kDNSType_SRV,  kHostNameTTL,
                                     kDNSRecordTypeUnregistered, AuthRecordAny, AutoTunnelRecordCallback, info);
            AssignConstStringDomainName(&info->AutoTunnelService.namestorage, "\x0B" "_autotunnel" "\x04" "_udp");
...
mDNSlocal void UpdateAutoTunnel6Record(mDNS *m, DomainAuthInfo *info)
{
    ...
    else if (info->AutoTunnel6Record.resrec.RecordType == kDNSRecordTypeUnregistered)
    {
        mDNS_SetupResourceRecord(&info->AutoTunnel6Record, mDNSNULL, mDNSInterface_Any, kDNSType_AAAA, kHostNameTTL,
                                 kDNSRecordTypeUnregistered, AuthRecordAny, AutoTunnelRecordCallback, info);
        AssignConstStringDomainName(&info->AutoTunnel6Record.namestorage, "\x0C" "_autotunnel6");
...
mDNSexport void AutoTunnelCallback(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    case TC_STATE_AAAA_PEER:
        ...
        if (!mDNSIPv6AddressIsZero(m->AutoTunnelRelayAddr))
        {
            ...
            AssignConstStringDomainName(&question->qname, "\x0C" "_autotunnel6");
        }
        else
        {
            ...
            AssignConstStringDomainName(&question->qname, "\x0B" "_autotunnel" "\x04" "_udp");
        }
...
#ifdef DNSSD_RELAY_CLIENT
	// Add Discovery Relay virtual interfaces...
	rciEnumerateInterfaces(m, utc, AddRelayLinkToList);
#endif
...
#ifdef DNSSD_RELAY_CLIENT
// Platform entry point used by Relay code to report link availability changes.
mDNSexport mStatus mDNSPlatformRefreshInterfaceList(mDNS *m)
{
	mDNS_Lock(m);
	SetNetworkChanged(10);
	mDNS_Unlock(m);
	return mStatus_NoError;
}
#endif
...
mDNSlocal void NetworkChanged(SCDynamicStoreRef store, CFArrayRef changedKeys, void *context)
{
    ...
    if (mDNS_LoggingEnabled)
    {
        ...
        LogInfo("***   Network Configuration Change   *** %d change%s %s%s%s%s%s%sdelay %d%s",
                c, c>1 ? "s" : "",
