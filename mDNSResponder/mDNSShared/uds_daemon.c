...
mDNSlocal void connection_termination(request_state *request)
{
    ...
    LogOperation("[R%d] DNSServiceCreateConnection STOP PID[%d](%s)", request->request_id, request->process_id, request->pid_name);
    ...
    {
        registered_record_entry *ptr = request->u.reg_recs;
        LogOperation("[R%d] DNSServiceRegisterRecord(%u %s) STOP PID[%d](%s)", request->request_id, ptr->key, RRDisplayString(&mDNSStorage, &ptr->rr->resrec), request->process_id, request->pid_name);
...
{
    request_state **req = &all_requests;
    LogDebug("[R%d] Cancel %08X %08X", request->request_id, request->hdr.client_context.u32[1], request->hdr.client_context.u32[0]);
    while (*req)
    {
        if ((*req)->primary == request &&
...
mDNSlocal mStatus handle_regrecord_request(request_state *request)
{
    ...
    if (rr)
    {
        ...
        LogOperation("[R%d] DNSServiceRegisterRecord(%u %s) START PID[%d](%s)", request->request_id, re->key,
                     RRDisplayString(&mDNSStorage, &rr->resrec), request->process_id, request->pid_name);

        err = mDNS_Register(&mDNSStorage, rr);
        if (err)
        {
            LogOperation("[R%d] DNSServiceRegisterRecord(%u %s) ERROR (%d)", request->request_id, re->key,
...
mDNSlocal void regservice_termination_callback(request_state *request)
{
    ...
    while (request->u.servicereg.instances)
    {
        ...
        LogOperation("[R%d] DNSServiceRegister(%##s, %u) STOP PID[%d](%s)", request->request_id, p->srs.RR_SRV.resrec.name->c,
...
mDNSlocal mStatus handle_add_request(request_state *request)
{
    ...
    if (!request->msgptr) { LogMsg("[R%d] DNSServiceAddRecord(unreadable parameters)", request->request_id); return(mStatus_BadParamErr); }
    ...
    { LogMsg("[R%d] DNSServiceAddRecord(not a registered service ref)", request->request_id); return(mStatus_BadParamErr); }
    ...
    { LogMsg("[R%d] DNSServiceAddRecord: adding record to a service registered with zero port", request->request_id); return(mStatus_BadParamErr); }

    LogOperation("[R%d] DNSServiceAddRecord(%X, %##s, %s, %d) PID[%d](%s)", request->request_id, flags,
...
mDNSlocal mStatus handle_update_request(request_state *request)
{
    ...
    if (!request->msgptr) { LogMsg("[R%d] DNSServiceUpdateRecord(unreadable parameters)", request->request_id); return(mStatus_BadParamErr); }
    ...
    if (request->terminate == connection_termination)
    {
        ...
        for (reptr = request->u.reg_recs; reptr; reptr = reptr->next)
        {
            if (reptr->key == hdr->reg_index)
            {
                ...
                LogOperation("[R%d] DNSServiceUpdateRecord(%##s, %s) PID[%d](%s)",
                             request->request_id, reptr->rr->resrec.name->c, reptr->rr ? DNSTypeName(reptr->rr->resrec.rrtype) : "<NONE>",
                             request->process_id, request->pid_name);
                goto end;
            }
        }
    ...
    if (request->terminate != regservice_termination_callback)
    { LogMsg("[R%d] DNSServiceUpdateRecord(not a registered service ref)", request->request_id); return(mStatus_BadParamErr); }

    // For a service registered with zero port, only SRV record is initialized. Don't allow any updates.
    if (mDNSIPPortIsZero(request->u.servicereg.port))
    { LogMsg("[R%d] DNSServiceUpdateRecord: updating the record of a service registered with zero port", request->request_id); return(mStatus_BadParamErr); }
...
mDNSlocal mStatus handle_removerecord_request(request_state *request)
{
    ...
    if (!request->msgptr) { LogMsg("[R%d] DNSServiceRemoveRecord(unreadable parameters)", request->request_id); return(mStatus_BadParamErr); }
    ...
    { LogMsg("[R%d] DNSServiceRemoveRecord(not a registered service ref)", request->request_id); return(mStatus_BadParamErr); }
    else
    {
        service_instance *i;
        mDNSu16 rrtype = 0;
        LogOperation("[R%d] DNSServiceRemoveRecord(%##s, %s) PID[%d](%s)", request->request_id,
                     (request->u.servicereg.instances) ? request->u.servicereg.instances->srs.RR_SRV.resrec.name->c : NULL,
...
mDNSlocal mStatus handle_regservice_request(request_state *request)
{
    ...
    LogOperation("[R%d] DNSServiceRegister(%X, %d, \"%s\", \"%s\", \"%s\", \"%s\", %u) START PID[%d](%s)",
                 request->request_id, request->flags, interfaceIndex, name, request->u.servicereg.type_as_string,
                 domain, host, mDNSVal16(request->u.servicereg.port), request->process_id, request->pid_name);
...
validReply:
    ...
    LogOperation("[R%d->Q%d] DNSServiceBrowse(%##s, %s) RESULT %s interface %d: %s",
                 req->request_id, mDNSVal16(question->TargetQID), question->qname.c, DNSTypeName(question->qtype), AddRecord ? "ADD" : "RMV",
...
mDNSlocal void browse_termination_callback(request_state *info)
{
    ...
    while (info->u.browser.browsers)
    {
        ...
        LogOperation("[R%d] DNSServiceBrowse(%X, %d, \"%##s\") STOP PID[%d](%s)",
                     info->request_id, info->flags, info->interfaceIndex, ptr->q.qname.c, info->process_id, info->pid_name);
...
mDNSlocal mStatus handle_browse_request(request_state *request)
{
    ...
    LogOperation("[R%d] DNSServiceBrowse(%X, %d, \"%##s\", \"%s\") START PID[%d](%s)",
                 request->request_id, request->flags, interfaceIndex, request->u.browser.regtype.c, domain,
                 request->process_id, request->pid_name);

    if (request->u.browser.default_domain)
    {
        // Start the domain enumeration queries to discover the WAB browse domains
        LogInfo("[R%d] DNSServiceBrowse Start WAB PID[%d](%s)", request->request_id, request->process_id, request->pid_name);
...
mDNSlocal void resolve_result_callback(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    *data++ =  req->u.resolve.srv->rdata->u.srv.port.b[1];
    ...
    LogOperation("[R%d->Q%d] DNSServiceResolve(%s) RESULT   %s:%d", req->request_id, mDNSVal16(question->TargetQID),
                 fullname, target, mDNSVal16(req->u.resolve.srv->rdata->u.srv.port));
    append_reply(req, rep);
}

mDNSlocal void resolve_termination_callback(request_state *request)
{
    LogOperation("[R%d] DNSServiceResolve(%X, %d, \"%##s\") STOP PID[%d](%s)",
                 request->request_id, request->flags, request->interfaceIndex, request->u.resolve.qtxt.qname.c,
                 request->process_id, request->pid_name);
...
mDNSlocal mStatus handle_resolve_request(request_state *request)
{
    ...
    // ask the questions
    LogOperation("[R%d] DNSServiceResolve(%X, %d, \"%##s\") START PID[%d](%s)", request->request_id, flags,
                 interfaceIndex, request->u.resolve.qsrv.qname.c, request->process_id, request->pid_name);
    ...
    if (!q->ForceMCast && SameDomainLabel(LastLabel(&q->qname), (const mDNSu8 *)&localdomain))
        if (q->qtype == kDNSType_A || q->qtype == kDNSType_AAAA || VALID_MSAD_SRV(q))
        {
            ...
            *q2               = *q;
            ...
            LogOperation("[R%d->Q%d] DNSServiceQueryRecord(%##s, %s) unicast", request->request_id, mDNSVal16(q2->TargetQID),
                         q2->qname.c, DNSTypeName(q2->qtype));
            err = mDNS_StartQuery(&mDNSStorage, q2);
            if (err)
                LogMsg("[R%d->Q%d] ERROR: DNSServiceQueryRecord %##s %s mDNS_StartQuery: %d",
                        request->request_id, mDNSVal16(q2->TargetQID), q2->qname.c, DNSTypeName(q2->qtype), (int)err);
        }
    return(err);
    ...
    DNSServiceErrorType error)
{
    ...
    LogOperation("[R%d->Q%d] %s(%##s, %s) RESULT %s interface %d: (%s)%s", req->request_id, mDNSVal16(question->TargetQID),
...
#if APPLE_OSX_mDNSResponder
    {
        ...
        if (q && question->qtype != q->qtype && !SameDomainName(&question->qname, &q->qname))
        {
            ...
            *question              = *q;
            ...
            LogOperation("[R%d->Q%d] DNSServiceQueryRecord(%##s, %s) unicast, context %p", req->request_id, mDNSVal16(question->TargetQID),
                         question->qname.c, DNSTypeName(question->qtype), question->QuestionContext);
...
{
    LogOperation("[R%d] DNSServiceQueryRecord(%X, %d, %##s, %s) STOP PID[%d](%s)",
        request->request_id, request->flags, request->interfaceIndex, request->u.queryrecord.q.qname.c, DNSTypeName(request->u.queryrecord.q.qtype), request->process_id, request->pid_name);
...
mDNSlocal mStatus handle_queryrecord_request(request_state *request)
{
    ...
    if (!request->msgptr)
    { LogMsg("[R%d] DNSServiceQueryRecord(unreadable parameters)", request->request_id); return(mStatus_BadParamErr); }

    request->flags = flags;
    request->interfaceIndex = interfaceIndex;
    mDNSPlatformMemZero(&request->u.queryrecord, sizeof(request->u.queryrecord));

    q->request_id          = request->request_id;
    ...
    LogOperation("[R%d] DNSServiceQueryRecord(%X, %d, %##s, %s) START PID[%d](%s)",
                 request->request_id, request->flags, interfaceIndex, q->qname.c, DNSTypeName(q->qtype),
                 request->process_id, request->pid_name);
    err = mDNS_StartQuery(&mDNSStorage, q);

    if (err)
    {
        LogMsg("[R%d] ERROR: DNSServiceQueryRecord %##s %s mDNS_StartQuery: %d",
               request->request_id, q->qname.c, DNSTypeName(q->qtype), (int)err);
    }
    else
    {
                                    ...
                                    DNSQuestion *const question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    LogOperation("[R%d->Q%d] DNSServiceEnumerateDomains(%#2s) RESULT %s: %s", request->request_id, mDNSVal16(question->TargetQID),
                 question->qname.c, AddRecord ? "ADD" : "RMV", domain);
...
mDNSlocal mStatus handle_release_request(request_state *request)
{
    ...
    LogOperation("[R%d] PeerConnectionRelease(%X %##s) START PID[%d](%s)",
                 request->request_id, flags, instance.c, request->process_id, request->pid_name);
...
mDNSlocal void handle_getproperty_request(request_state *request)
{
    ...
    {
        LogOperation("[R%d] DNSServiceGetProperty(%s)", request->request_id, prop);
...
mDNSlocal void handle_connection_delegate_request(request_state *request)
{
    ...
    LogOperation("[R%d] DNSServiceCreateDelegateConnection START PID[%d](%s)",
...
mDNSlocal void handle_getpid_request(request_state *request)
{
    ...
    LogMsg("[R%d] DNSServiceGetPID START", request->request_id);
...
{
    LogOperation("[R%d] DNSServiceNATPortMappingCreate(%X, %u, %u, %d) STOP PID[%d](%s)", request->request_id,
...
mDNSlocal void port_mapping_create_request_callback(mDNS *m, NATTraversalInfo *n)
{
    ...
    *data++ = request->u.pm.NATinfo.ExternalPort.b[1];
    ...
    LogOperation("[R%d] DNSServiceNATPortMappingCreate(%X, %u, %u, %d) RESULT %.4a:%u TTL %u", request->request_id,
                 DNSServiceProtocol(request->u.pm.NATinfo.Protocol),
                 mDNSVal16(request->u.pm.NATinfo.IntPort), mDNSVal16(request->u.pm.ReqExt), request->u.pm.NATinfo.NATLease,
...
mDNSlocal mStatus handle_port_mapping_request(request_state *request)
{
    ...
    if (!request->msgptr)
    { LogMsg("[R%d] DNSServiceNATPortMappingCreate(unreadable parameters)", request->request_id); return(mStatus_BadParamErr); }
    ...
    LogOperation("[R%d] DNSServiceNATPortMappingCreate(%X, %u, %u, %d) START PID[%d](%s)", request->request_id,
...
{
    LogOperation("[R%d] DNSServiceGetAddrInfo(%##s) STOP PID[%d](%s)", request->request_id, request->u.addrinfo.q4.qname.c,
                  request->process_id, request->pid_name);
...
mDNSlocal mStatus handle_addrinfo_request(request_state *request)
{
    ...
    request->u.addrinfo.q4.request_id         = request->u.addrinfo.q6.request_id         = request->request_id;
    ...
    LogOperation("[R%d] DNSServiceGetAddrInfo(%X, %d, %d, %##s) START PID[%d](%s)", 
        request->request_id, flags, interfaceIndex, request->u.addrinfo.protocol, d.c, request->process_id, request->pid_name);
...
mDNSlocal mStatus handle_client_request(request_state *req)
{
    ...
    switch(req->hdr.op)
    {
        ...
        case connection_request:
            LogOperation("[R%d] DNSServiceCreateConnection START PID[%d](%s)",
...
mDNSlocal void request_callback(int fd, short filter, void *info, mDNSBool encounteredEOF)
{
    mStatus err = 0;
...
mDNSlocal void connect_callback(int fd, short filter, void *info, mDNSBool encounteredEOF)
{
    dnssd_sockaddr_t cliaddr;
...
mDNSlocal void LogOneAuthRecord(const AuthRecord *ar, mDNSs32 now, const char *const ifname)
{
    ...
    {
        LogMsgNoIdent("%7d %7d %7d %-7s 0x%02X %s %s",
                      ar->ThisAPInterval / mDNSPlatformOneSecond,
...
mDNSexport void udsserver_info()
{
    ...
    else
    {
        ...
        for (q = m->Questions; q; q=q->next)
        {
            ...
            LogMsgNoIdent("%6d%6d %-7s%s%s %5d 0x%08x%08x%08x%08x 0x%p 0x%p %1d %2d  %-5s%##s%s",
                          i, n,
