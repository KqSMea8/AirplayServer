...
mDNSexport void SockAddrTomDNSAddr(const struct sockaddr *const sa, mDNSAddr *ipAddr, mDNSIPPort *ipPort)
{
    switch (sa->sa_family)
    {
    case AF_INET:
                                       ...
                                       mDNSIPPort dstPort, mDNSBool useBackgroundTrafficClass)
{
	...
	// Note that this does not use src or useBackgroundTrafficClass (of course, the main function doesn't either).
	if (thisIntf->coreIntf.discoveryRelayLink != NULL) {
		return rciSendMessage(msg, end, thisIntf->coreIntf.discoveryRelayLink);
	}
...
mDNSlocal void TCPReadCallback(int fd, short filter, void *context, mDNSBool encounteredEOF)
{
...
mDNSlocal void TCPWritableCallback(int fd, short filter, void *context, mDNSBool encounteredEOF)
{
	TCPSocket *sock = context;
	(void)fd;
	(void)filter;
	...
	sock->writable = mDNStrue;
}

mDNSlocal void TCPConnectCallback(int fd, short filter, void *context, mDNSBool encounteredEOF)
{
	TCPSocket *sock = context;
	...
	if (getsockopt(fd, SOL_SOCKET, SO_ERROR, &result, &len) < 0) {
		...
		if (result != 0) {
			...
			sock->connected = mDNStrue;
			sock->writable = mDNStrue; // Socket starts out writable.
			// Select for read events.
			sock->err = mDNSPlatformRequestReadEvents(sock->events.fd, "mDNSPosix::TCPConnectCallback", TCPReadCallback, sock, &sock->events);
			if (sock->err != mStatus_NoError) {
				LogMsg("mDNSPosix::TCPConnectCallback: Unable to request read events: %d", sock->err);
			}
		}
	}
											...
											domainname *hostname, mDNSBool useBackgroundTrafficClass)
{
	...
	// Create a socket
    sock->events.fd = socket(sa_family, SOCK_STREAM, IPPROTO_TCP);
	if (sock->events.fd < 0) {
		LogMsg("mDNSPosixTCPSocketBind: %s", strerror(errno));
		free(sock);
		return mDNSNULL;
	}
	...
	if (port != NULL) {
...
#ifndef NOT_HAVE_SA_LEN
		addr.sa.sa_len = (sa_family == AF_INET) ? sizeof (struct sockaddr_in) : sizeof (struct sockaddr_in6);
#endif
		...
		err = bind(sock->events.fd, (struct sockaddr *)&addr, sizeof addr);
		if (err < 0) {
			LogMsg("mDNSPosixTCPSocketBind getsockname: %s", strerror(errno));
			close(sock->events.fd);
			free(sock);
			return mDNSNULL;
		}

		// If we didn't specify a port, see what port we got.
		if (port->NotAnInteger == 0) {
			socklen_t addrlen = sizeof addr;
			err = getsockname(sock->events.fd, (struct sockaddr *)&addr, &addrlen);
			if (err < 0) {
				LogMsg("mDNSPosixTCPSocketBind getsockname: %s", strerror(errno));
				close(sock->events.fd);
				free(sock);
	...
	// do some pretty scary fu; I'm not sure what the benefit is, so leaving this out for now.
	(void)useBackgroundTrafficClass;
...
mDNSexport mStatus mDNSPlatformTCPSocketSetCallback(TCPSocket *sock, TCPConnectionCallback callback, void *context)
{
	...
	// Can't fail on Posix.
	return mDNSPlatformRequestReadEvents(sock->events.fd, "mDNSPlatformTCPSocketSetCallback",
										 TCPReadCallback, sock, &sock->events);
}
										  ...
										  mDNSInterfaceID InterfaceID, TCPConnectionCallback callback, void *context)
{
	...
	if (dst->type == mDNSAddrType_IPv4) {
		...
		memcpy(&addr.sin6.sin6_addr, &dst->ip.v6, sizeof addr.sin6.sin6_addr);
	}
#ifndef NOT_HAVE_SA_LEN
	addr.sa.sa_len = len;
#endif
	...
	if (result < 0) {
		if (errno == EINPROGRESS) {
			status = mDNSPlatformRequestWriteEvents(sock->events.fd,
													"mDNSPlatformConnect", TCPConnectCallback, sock, &sock->events);
			if (status != mStatus_NoError) {
				// This is a programming error, maybe should just assert().
				LogMsg("mDNSPlatformTCPConnect: programming error in write event setup.");
				return mStatus_UnknownErr;
			}
...
{
	if (sock) { // can sock really be NULL when this is called?
		shutdown(sock->events.fd, SHUT_RDWR);
		mDNSPlatformRemoveFDFromEventLoop(sock->events.fd, &sock->events);
		close(sock->events.fd);
		free(sock);
	}
}
	...
	*closed = mDNSfalse;
	if (sock->flags & kTCPSocketFlags_UseTLS) {
		...
		*closed = mDNStrue;
		...
		// These counters are copied from mDNSMacOSX.c.   I believe they are here so that if
		// the code that's using this TCP socket fails to handle a shut-down connection correctly,
		// mDNSResponder will eventually dump core and get restarted rather than just spinning
		// indefinitely, burning CPU.   IOW, this should never happen, but if it does, we get a
		// core dump to debug.
		static int CLOSEDcount = 0, EAGAINcount = 0;

		nread = read(sock->events.fd, buf, buflen);
		if (nread > 0) {
			CLOSEDcount = 0;	// Apparently we are making progress, so don't dump core.
			EAGAINcount = 0;
		} else if (nread == 0) {
			*closed = mDNStrue;
			if ((++CLOSEDcount % 1000) == 0) {
				LogMsg("ERROR: mDNSPlatformReadTCP: recv %d got %dth close: we are spinning.",
					   sock->events.fd, CLOSEDcount);
			...
			*closed = mDNStrue;
			...
			if ((++EAGAINcount % 1000) == 0) {
				LogMsg("ERROR: mDNSPlatformReadTCP - recv on %d got %dth EAGAIN: we are spinning.",
					   sock->events.fd, EAGAINcount);
				sleep(1);	// I wonder why mDNSMacOSX sleeps here instead of dumping core.
			}
		} else {
...
mDNSexport long mDNSPlatformWriteTCP(TCPSocket *sock, const char *msg, unsigned long len)
{
	...
	if (sock->flags & kTCPSocketFlags_UseTLS) {
		...
		result = write(sock->events.fd, msg, len);
        if (result < 0)
        {
	...
	// At this point we select again for write.   Normally we will immediately get a call back saying
	// that it's okay to write, but if we have filled the TCP output buffer to the value we set for
	// TCP_NOTSENT_LOWAT, then we won't get the callback until the buffer has drained below the low water
	// mark.
	sock->writable = mDNSfalse;
	err = mDNSPlatformRequestWriteEvents(sock->events.fd, "dpWriteFinish", TCPWritableCallback, sock, &sock->events);
	if (err != mStatus_NoError) {
		sock->err = err;
		LogMsg("mDNSPlatformWriteTCP: mDNSPlatformRequestWriteEvents returned %d", sock->err);
		nsent = -1;
	}
	return nsent;
}
...
mDNSlocal int SetupOneInterfaceOrLink(mDNS *const m, mDNSAddr *addr, mDNSAddr *mask, struct sockaddr *intfAddr, char *intfName, int intfIndex, mDNSs32 utc, void *link)
{
    int err = 0;
    PosixNetworkInterface *intf;
    ...
    if (err == 0)
    {
		...
		intf->coreIntf.ip = *addr;
		intf->coreIntf.mask = *mask;
    ...
    {
		if (link == NULL) {
			if (alias->multicastSocket4 == -1 && addr->type == mDNSAddrType_IPv4)
				err = SetupSocket(intfAddr, MulticastDNSPort, intf->index, &alias->multicastSocket4);
#if HAVE_IPV6
			else if (alias->multicastSocket6 == -1 && addr->type == mDNSAddrType_IPv6)
				err = SetupSocket(intfAddr, MulticastDNSPort, intf->index, &alias->multicastSocket6);
#endif
		} else {
			intf->coreIntf.discoveryRelayLink = link;
		}
    }
...
#endif
	if (link) {
		intf->coreIntf.SupportsUnicastMDNSResponse = mDNSfalse;
	} else {
...
// This is a shim that provides the function signature of the old SetupOneInterface function; that function now
// takes more mDNS-like arguments and supports registering Discovery Relay links as interfaces.
mDNSlocal int SetupOneInterface(mDNS *const m, struct sockaddr *intfAddr, struct sockaddr *intfMask, char *intfName, int intfIndex)
{
	mDNSAddr addr, mask;

	SockAddrTomDNSAddr(intfAddr, &addr, NULL);
	SockAddrTomDNSAddr(intfMask, &mask, NULL);
	return SetupOneInterfaceOrLink(m, &addr, &mask, intfAddr, intfName, intfIndex, 0, NULL);
}

// Call get_ifi_info() to obtain a list of active interfaces and call SetupOneInterface() or SetupOnePhysicalInterface on each one.
mDNSlocal int SetupInterfaceList(mDNS *const m)
{
    ...
    if (err == 0)
    {
		...
		// We now look for relay interfaces we already know about.  This is done by calling through to the
		// relay client interface code so that the posix code isn't reaching up into the relay protocol
		// code.
		rciEnumerateInterfaces(m, 0, SetupOneInterfaceOrLink);
...
mDNSlocal void InterfaceChangeCallback(int fd, short filter, void *context, mDNSBool encounteredEOF)
{
    ...
    if (changedInterfaces)
        mDNSPlatformRefreshInterfaceList(pChgRec->mDNS);
}
...
mDNSexport mStatus mDNSPlatformInit(mDNS *const m)
{
	...
	// Initialize Relay Proxy connection(s) if configured.
	rciInit(m);
...
mDNSexport mStatus mDNSPlatformRefreshInterfaceList(mDNS *const m)
{
    int err;
...
mDNSlocal void mDNSPosixGetFDSetWorker(mDNS *m, int *nfds, fd_set *readfds, fd_set *writefds)
{
	int fdm = *nfds;
...
mDNSexport void mDNSPosixGetFDSet(mDNS *m, int *nfds, fd_set *readfds, fd_set *writefds, struct timeval *timeout)
{
	...
	mDNSPosixGetFDSetWorker(m, nfds, readfds, writefds);
}
...
mDNSexport void mDNSPosixProcessFDSet(mDNS *const m, fd_set *readfds, fd_set *writefds)
{
	...
	for (iSource = gEventSources; iSource; iSource = iSource->next)
	{
		...
		{
			iSource->readCallback(iSource->fd, 0, iSource->readContext, mDNSfalse);
		...
		{
			mDNSPlatformEventCallback writeCallback = iSource->writeCallback;
			...
			writeCallback(iSource->fd, 0, iSource->writeContext, mDNSfalse);
			break;  // in case callback removed elements from gEventSources
		}
	}
}

unsigned mDNSPlatformEventContextSize(void)
{
	return sizeof (PosixEventSource);
}

mDNSlocal mStatus requestIOEvents(int fd, const char *taskName, mDNSPlatformEventCallback callback,
						void *context, void *eventContext, int flag)
{
    PosixEventSource *newSource = eventContext;
...
// Allocate a structure and then call mDNSPlatformAddFDToEventLoop on it.
mStatus mDNSPosixAddFDToEventLoop(int fd, mDNSPlatformEventCallback callback, void *context)
{
	mStatus status;
	...
	status = mDNSPlatformRequestReadEvents(fd, "mDNSPosixAddFDToEventLoop", callback, context, newSource);
	if (status != mStatus_NoError)
		free(newSource);
	return status;
}

mStatus mDNSPlatformRequestReadEvents(int fd, const char *taskName, mDNSPlatformEventCallback callback, void *context, void *eventContext)
{
	return requestIOEvents(fd, taskName, callback, context, eventContext, PosixEventFlag_Read);
}

mStatus mDNSPlatformRequestWriteEvents(int fd, const char *taskName, mDNSPlatformEventCallback callback, void *context, void *eventContext)
{
	return requestIOEvents(fd, taskName, callback, context, eventContext, PosixEventFlag_Write);
}

// Remove a file descriptor from the set that mDNSPosixRunEventLoopOnce() listens to.
mDNSlocal mStatus stopReadOrWriteEvents(int fd, mDNSBool freeContext, mDNSBool removeContext,
										int flags, void *eventContext)
{
...
// This version is called by callers who want the Posix version of the API, which allocates the event structure
// and frees it automatically.
mStatus mDNSPosixRemoveFDFromEventLoop(int fd)
{
	return stopReadOrWriteEvents(fd, mDNStrue, mDNStrue, PosixEventFlag_Read | PosixEventFlag_Write, NULL);
}

// Stop listening for read events on fd.   Event context is not freed.
mStatus mDNSPlatformStopReadEvents(int fd, void *context)
{
	return stopReadOrWriteEvents(fd, mDNSfalse, mDNSfalse, PosixEventFlag_Read, context);
}

// Stop listening for read events on fd.   Event context is not freed.
mStatus mDNSPlatformStopWriteEvents(int fd, void *context)
{
	return stopReadOrWriteEvents(fd, mDNSfalse, mDNSfalse, PosixEventFlag_Write, context);
}

// Remove the event structure from the list
mStatus mDNSPlatformRemoveFDFromEventLoop(int fd, void *context)
{
	return stopReadOrWriteEvents(fd, mDNSfalse, mDNStrue, PosixEventFlag_Read | PosixEventFlag_Write, context);
}
								  ...
								  sigset_t *pSignalsReceived, mDNSBool *pDataDispatched)
{
	...
	// In this case this is already done.

    // Include the sockets that are listening to the wire in our select() set
    mDNSPosixGetFDSetWorker(m, &fdMax, &listenFDs, &writeFDs);
    numReady = select(fdMax, &listenFDs, &writeFDs, (fd_set*) NULL, &timeout);
