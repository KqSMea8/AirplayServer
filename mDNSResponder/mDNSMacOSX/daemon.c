...
#include "RelayInterface.h"         // Prototypes for the Discovery Relay interface
...
mDNSlocal void PrepareForIdle(void *m_param)
{
	...
	nextTimerEvent         = rciIdle(m, nextTimerEvent);
#endif
...
mDNSlocal void kqUDSEventCallback(int fd, short filter, void *context, mDNSBool encounteredEOF)
{
    ...
    source->callback(fd, filter, source->context, encounteredEOF);
}
